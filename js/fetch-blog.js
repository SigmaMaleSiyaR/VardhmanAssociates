        ...
        <div class="col" style="min-width: 300px; max-width: 350px; padding: 0 0.5rem;">
            <div class="card h-100 shadow-sm" style="border-radius: 8px; transition: all 0.3s ease; cursor: pointer;" 
                ...
                <div style="height: 150px; overflow: hidden; border-radius: 8px 8px 0 0;">
                    <img src="${
                      blog.imageUrl || "https://via.placeholder.com/300x150"
                    }" 
                         class="card-img-top" 
                         alt="${blog.title}" 
                ...
                <div class="card-body" style="padding: 1rem;">
                    <h5 class="card-title text-truncate mb-2" style="font-size: 1.1rem; font-weight: 600;" title="${
                      blog.title || "Untitled"
                    }">
                        ${blog.title || "Untitled"}
                    </h5>
...
async function loadBlogs(limit = null) {
  ...
  try {
    ...
    if (querySnapshot.empty) {
      ...
      container.innerHTML =
        '<div class="col-12 text-center"><p class="text-muted">No blogs found.</p></div>';
      return;
    }
    ...
    querySnapshot.forEach((doc) => {
      ...
      cardElement.addEventListener("click", () => {
        console.log("Blog card clicked:", blog, "ID:", doc.id);
    ...
    container.innerHTML = container.innerHTML =
      '<div class="col-12 text-center"><p class="text-danger">Error loading blogs. Please try again later.</p></div>';
  } finally {
    spinner.classList.add("d-none");
  }
}

// Load blogs when the page loadsLoad blogs when the page loads
document.addEventListener("DOMContentLoaded", () => {document.addEventListener("DOMContentLoaded", () => {




});  loadBlogs();  console.log("Page loaded, initializing Firebase...");  console.log("Page loaded, initializing Firebase...");
  loadBlogs();
});
